"""Train the CNN ECG classifier on synthetic data or PTB-XL."""
...
from data.data_loader import (
    ...
    get_cnn_dataloaders,
    load_ptbxl_cnn_dataframe,
)


def train_one_epoch(model, loader, optimizer, criterion, device):
    """Run one training epoch and return mean loss and accuracy."""
    model.train()
    ...
    for batch_idx, (images, labels) in enumerate(loader):
        ...
        # Keep updates stable on noisy PTB-XL batches.
        ...
        if (batch_idx + 1) % 10 == 0:
            print(f"  Step [{batch_idx+1}/{len(loader)}] " f"Loss: {loss.item():.4f}")
...
def evaluate(model, loader, criterion, device):
    """Evaluate the model and return loss, accuracy, labels, predictions, and scores."""
    model.eval()
    total_loss, correct, total = 0.0, 0, 0
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        roc_auc_display = (
            "N/A" if val_metrics["roc_auc"] is None else f"{val_metrics['roc_auc']:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-samples", type=int, default=1000, help="Synthetic samples to generate"
    )
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument(
        "--real-data",
        action="store_true",
        help="Load real PTB-XL data instead of synthetic data",
    )
    parser.add_argument(
        "--ptbxl-dir",
        type=str,
        default=None,
        help="Path to the extracted PTB-XL dataset",
    )
    parser.add_argument(
        "--sampling-rate",
        type=int,
        default=100,
        choices=[100, 500],
        help="PTB-XL signal sampling rate",
    )
    parser.add_argument(
        "--lead-index",
        type=int,
        default=1,
        help="Lead index to load from each PTB-XL record",
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="Optional cap on loaded PTB-XL records"
    )
